...
图片推理会对常见白底黑字进行反色，再缩放并居中到 28×28；较复杂背景、多个数字、裁剪不全的图片可能识别错误。所示置信度是 softmax 输出，不代表经校准的概率。训练和测试均使用 MNIST 均值、标准差归一化。CNN 由两层卷积与池化、批归一化、Dropout 和全连接层构成。
...
CI 运行离线单元测试。训练准确率随随机性、设备与轮次而变；仓库没有预先测得的准确率或可直接使用的模型文件。模型文件、MNIST 数据及生成结果由 .gitignore 排除。Docker 使用前也须提供已训练的权重，详见 Dockerfile。
